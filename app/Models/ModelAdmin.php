<?php

namespace App\Models;

use CodeIgniter\Model;

class ModelAdmin extends Model
{
    public function dataSetting(){
        return $this->db->table('tbl_setting')
        ->where('id_setting', 1)->get()->getRowArray();
    }

    public function updateSetting($data){
        $this->db->table('tbl_setting')
        ->where('id_setting', 1)
        ->update($data);
    }

    public function getIzinPending()
{
    return $this->db->table('tbl_izin')
        ->join('tbl_karyawan', 'tbl_karyawan.id_karyawan = tbl_izin.id_karyawan')
        ->where('status_approved', 0) // waiting
        ->get()->getResultArray();
}

    public function countIzinByStatus($status)
{
    return $this->db->table('tbl_izin')
        ->where('status_approved', $status)
        ->countAllResults();
}

    public function countAllIzin()
{
    return $this->db->table('tbl_izin')->countAllResults();
}

}
